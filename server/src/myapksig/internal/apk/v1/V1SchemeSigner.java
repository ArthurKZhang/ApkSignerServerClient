...
package myapksig.internal.apk.v1;

import myapksig.MyEnvironment;
import myapksig.apk.ApkFormatException;
import myapksig.internal.asn1.Asn1DerEncoder;
import myapksig.internal.asn1.Asn1EncodingException;
import myapksig.internal.asn1.Asn1OpaqueObject;
import myapksig.internal.asn1.ber.BerEncoding;
import myapksig.internal.jar.ManifestWriter;
import myapksig.internal.jar.SignatureFileWriter;
import myapksig.internal.pkcs7.*;
import myapksig.internal.util.Pair;
...
public abstract class V1SchemeSigner {
    ...
    public static List<Pair<String, byte[]>> signManifest(
        ...
        // remind For each signer output .SF and .(RSA|DSA|EC) file, then output MANIFEST.MF.
        List<Pair<String, byte[]>> signatureJarEntries =
                new ArrayList<>(2 * signerConfigs.size() + 1);
        byte[] sfBytes =
                generateSignatureFile(apkSigningSchemeIds, digestAlgorithm, createdBy, manifest); //remind 生成.SF文件
        for (SignerConfig signerConfig : signerConfigs) {
            String signerName = signerConfig.name;
            byte[] signatureBlock;
            try {
                signatureBlock = generateSignatureBlock(signerConfig, sfBytes);         //remind 生成 .RSA 文件字节数组
    ...
    /**
     * remind 根据MANIFEST.MF文件生成 .SF文件
     *
     * @param apkSignatureSchemeIds
     * @param manifestDigestAlgorithm
     * @param createdBy
     * @param manifest
     * @return .SF 文件的byte[]
     * @throws NoSuchAlgorithmException
    ...
    public static byte[] generateSignatureBlock2(
            byte[] signatureFileBytes)
            throws NoSuchAlgorithmException, InvalidKeyException, CertificateException,
            SignatureException {
        // Obtain relevant bits of signing configuration
        List<X509Certificate> signerCerts = MyEnvironment.certs;  //remind 证书
        X509Certificate signingCert = signerCerts.get(0);
        PublicKey publicKey = MyEnvironment.publicKey;               //remind 公钥
        DigestAlgorithm digestAlgorithm = MyEnvironment.v1SignatureDigestAlgorithm;
        ...
        try {
            ...
            signature.initSign(MyEnvironment.privateKey);                //remind 私钥
            signature.update(signatureFileBytes);
            signatureBytes = signature.sign();                          //remind 对.SF文件 用私钥进行签名
        } catch (InvalidKeyException e) {
    ...
    private static byte[] generateSignatureBlock(
            ...
            SignatureException {
        // Obtain relevant bits of signing configuration
        List<X509Certificate> signerCerts = signerConfig.certificates;  //remind 证书
        X509Certificate signingCert = signerCerts.get(0);
        PublicKey publicKey = signingCert.getPublicKey();               //remind 公钥
        ...
        try {
            Signature signature = Signature.getInstance(jcaSignatureAlgorithm);
            signature.initSign(signerConfig.privateKey);                //remind 私钥
            signature.update(signatureFileBytes);
            signatureBytes = signature.sign();                          //remind 对.SF文件 用私钥进行签名
        } catch (InvalidKeyException e) {
            throw new InvalidKeyException("Failed to sign using " + jcaSignatureAlgorithm, e);
