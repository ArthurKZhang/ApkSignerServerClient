...
import myapksig.apk.ApkFormatException;
import myapksig.apk.ApkSigningBlockNotFoundException;
import myapksig.apk.ApkUtils;
import myapksig.apk.MinSdkVersionException;
import myapksig.internal.util.ByteBufferDataSource;
import myapksig.internal.zip.CentralDirectoryRecord;
import myapksig.internal.zip.EocdRecord;
import myapksig.internal.zip.LocalFileRecord;
import myapksig.internal.zip.ZipUtils;
import myapksig.util.*;
import myapksig.zip.ZipFormatException;
...
public class ApkSigner {
    ...
    private void sign(
        ...
        //Remind inputApkLfhSection是第一块: "Contents of ZIP entries" Lfh Section?
        //签名后apk内的分块: "1. Contents of ZIP entries; 2. APK signing block; 3. central directory; 4. End of Central Directory"
        ...
        List<Hints.PatternWithRange> pinPatterns = extractPinPatterns(  //remind assets/com.android.hints.pins.txt 是什么文件？
        ...
        for (final CentralDirectoryRecord inputCdRecord : inputCdRecordsSortedByLfhOffset) {
            ...
            LocalFileRecord inputLocalFileRecord; //Remind a file in the Zip pack
            try {
            ...
            if (shouldOutput) {
                ...
                inspectEntryRequest = signerEngine.outputJarEntry(entryName);//Remind get dataDigestRequest!!
                if (inspectEntryRequest != null) {
                    fulfillInspectInputJarEntryRequest(
                            inputApkLfhSection, inputLocalFileRecord, inspectEntryRequest);
                }
        ...
        }//End for loop. adding records in outputCdRecordsByName Map.
        ...
        if (inputOffset < inputLfhSectionSize) {
            ...
            inputApkLfhSection.feed(inputOffset, chunkSize, outputApkOut); //remind 最后一次使用 inputApkLfhSection(DataSource) 的地方
            outputOffset += chunkSize;
        ...
        if (outputJarSignatureRequest != null) {
            ...
            }//end for loop ApkSignerEngine.OutputJarSignatureRequest.JarEntry entry : outputJarSignatureRequest.getAdditionalJarEntries()
    ...
    /**
     * 将 localFileRecord 的解压后的数据放入 inspectEntryRequest 对象中
     * @param lfhSection
     * @param localFileRecord
     * @param inspectEntryRequest
     * @throws IOException
    ...
    private static void fulfillInspectInputJarEntryRequest(
        ...
        inspectEntryRequest.done(); //Remind related to the Digest
    }
    ...
    static ByteBuffer getAndroidManifestFromApk(
            ...
            DataSource lhfSection) throws IOException, ApkFormatException, ZipFormatException {
    ...
    private static int getMinSdkVersionFromApk(
            List<CentralDirectoryRecord> cdRecords,
            DataSource lhfSection) throws IOException, MinSdkVersionException {
