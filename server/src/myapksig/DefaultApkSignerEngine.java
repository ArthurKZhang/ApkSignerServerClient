...
import myapksig.internal.apk.ApkSigningBlockUtils;
import myapksig.internal.apk.SignatureAlgorithm;
import myapksig.internal.apk.v1.DigestAlgorithm;
import myapksig.internal.apk.v1.V1SchemeSigner;
import myapksig.internal.apk.v1.V1SchemeVerifier;
import myapksig.internal.apk.v2.V2SchemeSigner;
import myapksig.internal.apk.v3.V3SchemeSigner;
import myapksig.internal.jar.ManifestParser;
import myapksig.internal.util.AndroidSdkVersion;
import myapksig.internal.util.Pair;
import myapksig.internal.util.TeeDataSink;
import myapksig.util.DataSink;
import myapksig.util.DataSinks;
import myapksig.util.DataSource;
import myapksig.util.RunnablesExecutor;
...
public class DefaultApkSignerEngine implements ApkSignerEngine {
    ...
    public void inputApkSigningBlock(DataSource apkSigningBlock) {
        ...
        if (mOtherSignersSignaturesPreserved) {
            // TODO: Preserve blocks other than APK Signature Scheme v2 blocks of signers configured in this engine.
            return;
        }
    ...
    public OutputJarSignatureRequest outputJarEntries()
        ...
        for (GetJarEntryDataDigestRequest digestRequest
            ...
            //Remind 名字和digest的组合集合，应该要输出这个倒Manifest文件
        ...
        if ((mAddV1SignatureRequest == null) || (!mAddV1SignatureRequest.isDone())) {
            try {
                signatureZipEntries =
                        V1SchemeSigner.sign(                //remind 1  return V1SchemeSigner.signManifest()
                                mV1SignerConfigs,
            ...
            V1SchemeSigner.OutputManifestFile newManifest =
                    V1SchemeSigner.generateManifestFile(          //remind 生成 MANIFEST.MF文件
                            mV1ContentDigestAlgorithm,
                            mOutputJarEntryDigests,
                            inputJarManifest);
            ...
            if (!Arrays.equals(newManifest.contents, emittedSignatureManifest)) {
                ...
                try {
                    signatureZipEntries =
                            V1SchemeSigner.signManifest(       //remind 2
                                    mV1SignerConfigs,
                                    mV1ContentDigestAlgorithm,
                                    apkSigningSchemeIds,
                                    mCreatedBy,
                ...
                // output.
                signatureZipEntries = new ArrayList<>();        //remind 3
    ...
    private OutputApkSigningBlockRequestImpl outputZipSectionsInternal(
            ...
            DataSource zipCentralDirectory,
            DataSource zipEocd,
            boolean apkSigningBlockPaddingSupported) throws IOException, InvalidKeyException, SignatureException, NoSuchAlgorithmException {
        ...
        // Remind create APK Signing Block with v2 and/or v3 blocks
